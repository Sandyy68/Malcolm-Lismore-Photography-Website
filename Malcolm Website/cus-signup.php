...
<div class="lgbody">
  <div style="background-color: #030303b3; width: 325px; margin: auto; margin-top: 100px; padding:50px; border-radius: 20px;  text-align: center; -webkit-box-shadow: 0px 0px 24px 7px rgba(0,0,0,0.66); -moz-box-shadow: 0px 0px 24px 7px rgba(0,0,0,0.66);box-shadow: 0px 0px 24px 7px rgba(0,0,0,0.66);">
    <form action="cus-signup.php" method="POST" >
      <h1 style="color:white">Customer Sign Up</h1>
      ...
      <button class="btn btn-success w-100" onclick="return UserLoginConfirm()">Sign Up</button>
      <br><br>
    </form>
  </div>
</div>
<!--cus sign up data base handle-->
<?php 
    if(isset($_POST["uname"])){
      ...
      $db_name = "lismore_db";
        ...
        if (!$conn) {
            ...
            $sql = "INSERT INTO `login_table`( `u_name`, `u_psw`) VALUES ('$unme','$upsw')";

            if ($conn->query($sql) === TRUE) {
                echo "<script>window.location.href='customer-login.php';alert('Successfull! Now you can Login!');</script>";
            } else {
