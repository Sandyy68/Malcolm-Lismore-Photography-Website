...
<?php 
    if(isset($_POST["txtPackName"])){
        ...
        if (!$conn) {
            ...
            $sql = "UPDATE `packages-table` SET `pack_Name`='$PackName',`pack_Price`='$PackPrice' WHERE `pack_id`='$packID'";

            if ($conn->query($sql) === TRUE) {
                
                echo "<script>window.location.href='admin-dashboard.php';alert('Package Updated Successfully!');</script>";
