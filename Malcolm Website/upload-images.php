...
if (isset($_FILES["ppic"]) && $_FILES["ppic"]["error"] == 0) {
    ...
    if (in_array($filetype, $allowed)) {
        ...
        if (file_exists("pic/" . $filename)) {
            ...
            //recod insert part
                $sql = "INSERT INTO `gallery_table`(`gal_location`, `gal_type`) VALUES ('$propic1','$imgType')";

                if ($conn->query($sql) === TRUE) {
                    echo "<script>window.location.href='admin-dashboard.php';alert('New Image added successfully');</script>";
                } else {
