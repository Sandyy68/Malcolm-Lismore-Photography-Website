...
<div class="lgbody">
  <div style="background-color: #030303b3; width: 325px; margin: auto; margin-top: 100px; padding:50px; border-radius: 20px;  text-align: center; -webkit-box-shadow: 0px 0px 24px 7px rgba(0,0,0,0.66); -moz-box-shadow: 0px 0px 24px 7px rgba(0,0,0,0.66);box-shadow: 0px 0px 24px 7px rgba(0,0,0,0.66);">
    <form action="customer-login.php" method="POST" >
      ...
      <br><br>
      <div>
          <a href="cus-signup.php"> Click here to create an account</a>
      </div>
      <button class="btn btn-success w-100" onclick="return CusLoginConfirm()">Login</button>
      <br><br>
    </form>
  </div>
</div>
<!--Customer login data base handle-->
<?php 
if(isset($_POST["uname"])){
    ...
    if (!$conn){
        ...
        $sql = "SELECT * From login_table where u_name='". $unme . "' and u_psw='" . $upsw . "'";
        ...
        if($result->num_rows > 0){
            ...
            while($row = $result->fetch_assoc()){
                ...
                $redirectUrl = "cus-dashboard.php?userid=" . urlencode($uid);
            ...
            echo "<script>window.location.href='customer-login.php';alert('Wrong username or password.');</script>";
